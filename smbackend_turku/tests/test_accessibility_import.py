...
from smbackend_turku.tests.test_units_import import get_test_resource


@pytest.mark.django_db
@patch("smbackend_turku.importers.utils.get_turku_resource")
def create_units(turku_resource):
    from smbackend_turku.importers.units import UnitImporter

    logger = logging.getLogger(__name__)
    unit_importer = UnitImporter(logger=logger)

    turku_resource.return_value = get_test_resource(resource_name=None)
...
def test_accessibility_variables_import(ar_resource, ar_se_resource, ar_se_accessibility_resource):
    ...
    unit_accessibility_properties = UnitAccessibilityProperty.objects.all()
    unit_accessibility_property_1 = UnitAccessibilityProperty.objects.get(id=1)
    unit_accessibility_property_2 = UnitAccessibilityProperty.objects.get(id=5)

    assert len(unit_accessibility_properties) == 5
    assert unit_accessibility_property_1.unit == unit_identifier_1.unit
    assert unit_accessibility_property_2.unit == unit_identifier_2.unit
    assert unit_accessibility_property_1.variable_id == 259
    assert unit_accessibility_property_2.variable_id == 72
